...
public class MachineService extends UnicastRemoteObject implements IDao<Machine> {
    private Object HibernateConfig;
    ...
    @Override
    public List<Machine> findMachinesBySalle(int salleId) throws RemoteException {
    Session session = null;
    List<Machine> machines = null;
    Transaction tx = null;
    try {
       session = HibernateUtil.getSessionFactory().openSession();
       tx = session.beginTransaction();
       machines = session.getNamedQuery("findMachinesBySalle").setParameter("salleId", salleId).list();
       tx.commit();
    } catch (HibernateException ex) {
        if (tx != null) {
                tx.rollback();
